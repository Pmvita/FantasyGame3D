            ...
            if (className) {
                try {
                    ...
                    const attachEquipment = (equipmentMesh, equipmentName) => {
                        ...
                        // 1. Pre-process equipment: Center pivot and calculate base size
                        // We do this BEFORE scaling to ensure we have accurate dimensions
                        equipmentClone.updateMatrixWorld(true);
                        const box = new THREE.Box3().setFromObject(equipmentClone);
                        const size = box.getSize(new THREE.Vector3());
                        const center = box.getCenter(new THREE.Vector3());

                        const itemHeight = size.y;
                        const itemMaxDim = Math.max(size.x, size.y, size.z);

                        // For static items (weapon/helmet), we want the pivot to be at a sensible place
                        const isStatic = equipmentName === 'weapon' || equipmentName === 'helmet';
                        if (isStatic) {
                            // Move internal meshes so origin (0,0,0) is at the center (or bottom for helmet)
                            const offset = new THREE.Vector3().copy(center).negate();
                            if (equipmentName === 'helmet') {
                                // Helmet origin at bottom (neck)
                                offset.y = -box.min.y;
                            }
                        ...
                        // 2. Try to bind skeleton (primarily for armor)
                        ...
                        // Match character's base dimensions (at scale 1,1,1)
                        ...
                        if (equipmentName === 'weapon') {
                            // Target weapon size: ~40% of character height
                            const targetWeaponSize = charHeight * 0.4;
                            if (itemMaxDim > 0) normalizationScale = targetWeaponSize / itemMaxDim;
                        } else if (equipmentName === 'helmet') {
                            // Target helmet size: ~15% of character height
                            const targetHelmetSize = charHeight * 0.15;
                            if (itemHeight > 0) normalizationScale = targetHelmetSize / itemHeight;
                        } else {
                            // Armor: Match character height
                            if (itemHeight > 0) normalizationScale = charHeight / itemHeight;
                        }

                        // 4. Determine Attachment Point
                        let attachmentParent = model;
                        if (equipmentName === 'weapon') {
                            attachmentParent = findBone(characterSkeleton, 'RightHand') ||
                                findBone(characterSkeleton, 'Hand_R') ||
                                findBone(characterSkeleton, 'Hand.R') ||
                                model;
                        } else if (equipmentName === 'helmet') {
                            attachmentParent = findBone(characterSkeleton, 'Head') || model;
                        } else if (equipmentName === 'armour' && !skeletonBound) {
                            // Fallback for static armor: Attach to chest/spine
                            attachmentParent = findBone(characterSkeleton, 'Spine2') ||
                                findBone(characterSkeleton, 'Spine1') ||
                                findBone(characterSkeleton, 'Chest') ||
                                model;
                        }

                        // 5. Final Scale and Position
                        // Bone-attached items inherit characterScaleFactor from the model root
                        let finalScale = normalizationScale;
                        if (attachmentParent === model) {
                            finalScale *= characterScaleFactor;
                        }
                        ...
                        if (attachmentParent !== model) {
                            ...
                            if (equipmentName === 'weapon') {
                                // Default weapon rotation: Handle usually points along axis
                                equipmentClone.rotation.set(-Math.PI / 2, Math.PI / 2, 0);
                            }
                        } else {
                            // Root-attached (rigged) items should stay at origin
                            equipmentClone.position.set(0, 0, 0);
                            equipmentClone.rotation.set(0, 0, 0);
                        }

                        // 6. Clean up: Hide oversized meshes that might be artifacts
                        equipmentClone.traverse(child => {
                            if (child.isMesh) {
                                child.castShadow = true;
                                child.receiveShadow = true;
                                // Hide huge planes or boxes that might be model artifacts
                                const childBox = new THREE.Box3().setFromObject(child);
                                const childSize = childBox.getSize(new THREE.Vector3());
                                if (childSize.length() > 50) {
                                    child.visible = false;
                                }
                            }
                        });

                        attachmentParent.add(equipmentClone);
                        equipmentClone.updateMatrixWorld(true);

                        console.log(`Attached ${equipmentName} to ${attachmentParent.name || 'root'}:`, {
                            skeletonBound,
                            normalizationScale: normalizationScale.toFixed(3),
                            finalScale: finalScale.toFixed(3),
                            boneFound: attachmentParent !== model
                        });
                    ...
                    if (equipment.armour) attachEquipment(equipment.armour, 'armour');
                    if (equipment.boots) attachEquipment(equipment.boots, 'boots');
                    if (equipment.cape) attachEquipment(equipment.cape, 'cape');
                    if (equipment.helmet) attachEquipment(equipment.helmet, 'helmet');
                    if (equipment.weapon) attachEquipment(equipment.weapon, 'weapon');
                } catch (error) {
